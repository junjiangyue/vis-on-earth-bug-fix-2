...
class PieChart {
  ...
  private:
    ...
    class PerVolParam {
      ...
      public:
        ...
        PerVolParam(decltype(pieDat) pieDat, PerPlotParam *ploter)
        ...
        osg::Geode *MakePieChart(float fontSize = osg::WGS_84_RADIUS_EQUATOR / 40) {
            ...
            bottomGeom->addPrimitiveSet(
                new osg::DrawArrays(osg::PrimitiveSet::POLYGON, 0, bVer->size()));
            ...
            pcGeode->getOrCreateStateSet()->setAttribute(new osg::LineWidth(3.0),
                                                         osg::StateAttribute::ON);
            ...
            for (size_t i = 0; i < pieDat->size(); ++i) {
                ...
                osg::Vec3f point2 =
                    osg::Vec3f(bottomCenterPoint.x() + 0.5 * sinf(deg2Rad(startAngle)),
                               bottomCenterPoint.y() + 0.5 * cosf(deg2Rad(startAngle)), prop);
                osg::Vec3f point3 =
                    osg::Vec3f(bottomCenterPoint.x() + 0.5 * sinf(deg2Rad(startAngle)),
                               bottomCenterPoint.y() + 0.5 * cosf(deg2Rad(startAngle)), 0.0);

                osg::Vec3f point4 = osg::Vec3f(
                    bottomCenterPoint.x() + 0.5 * sinf(deg2Rad(startAngle + prop * 360.0)),
                    bottomCenterPoint.y() + 0.5 * cosf(deg2Rad(startAngle + prop * 360.0)), prop);
                osg::Vec3f point5 = osg::Vec3f(
                    bottomCenterPoint.x() + 0.5 * sinf(deg2Rad(startAngle + prop * 360.0)),
                    bottomCenterPoint.y() + 0.5 * cosf(deg2Rad(startAngle + prop * 360.0)), 0.0);
                ...
                for (float i = startAngle; i < startAngle + prop * 360.0; i += step) {
                    ...
                    pGeom->addPrimitiveSet(
                        new osg::DrawArrays(osg::PrimitiveSet::POLYGON, 0, pVer->size()));

                    osg::ref_ptr<osg::Vec3Array> normals = new osg::Vec3Array;

                    normals->push_back(vec3ToSphere(osg::Vec3(0.5, 0.5, 1.0)) -
                                       vec3ToSphere(osg::Vec3(0.5, 0.5, 0.0)));
                ...
                for (int i = 0; i <= 50; ++i) {
                    pVer0->push_back(vec3ToSphere(osg::Vec3f(
                        bottomCenterPoint.x() + 0.5 * sinf(deg2Rad(startAngle)) * i / 50.0,
                        bottomCenterPoint.y() + 0.5 * cosf(deg2Rad(startAngle)) * i / 50.0, 0.0)));
                }
                ...
                pGeom0->addPrimitiveSet(
                    new osg::DrawArrays(osg::PrimitiveSet::POLYGON, 0, pVer0->size()));

                osg::ref_ptr<osg::Vec3Array> normal0 = new osg::Vec3Array;

                normal0->push_back(
                    vec3ToSphere(osg::Vec3f(
                        bottomCenterPoint.x() + 0.5 * sinf(deg2Rad(startAngle - 90.0)),
                        bottomCenterPoint.y() + 0.5 * cosf(deg2Rad(startAngle - 90.0)), 0.0)) -
                    vec3ToSphere(osg::Vec3(0.5, 0.5, 0.0)));
                ...
                // pVer0->push_back(vec3ToSphere(point1));
                for (int i = 0; i <= 50; ++i) {
                    pVer1->push_back(vec3ToSphere(
                        osg::Vec3f(bottomCenterPoint.x() +
                                       0.5 * sinf(deg2Rad(startAngle + prop * 360.0)) * i / 50.0,
                                   bottomCenterPoint.y() +
                                       0.5 * cosf(deg2Rad(startAngle + prop * 360.0)) * i / 50.0,
                                   0.0)));
                }
                ...
                pGeom1->addPrimitiveSet(
                    new osg::DrawArrays(osg::PrimitiveSet::POLYGON, 0, pVer1->size()));

                osg::ref_ptr<osg::Vec3Array> normal1 = new osg::Vec3Array;
                normal1->push_back(vec3ToSphere(osg::Vec3f(
                                       bottomCenterPoint.x() +
                                           0.5 * sinf(deg2Rad(startAngle + prop * 360.0 + 90.0)),
                                       bottomCenterPoint.y() +
                                           0.5 * cosf(deg2Rad(startAngle + prop * 360.0 + 90.0)),
                                       0.0)) -
                                   vec3ToSphere(osg::Vec3(0.5, 0.5, 0.0)));
                // normals->push_back(vec3ToSphere(loc1) - vec3ToSphere(osg::Vec3(0.5, 0.5, 0.0)));
                ...
                for (int i = 0; i < 50; ++i) {
                    ...
                    osg::Vec3f loc0 =
                        osg::Vec3f(bottomCenterPoint.x() +
                                       0.5 * sinf(deg2Rad(startAngle + i * prop * 360.0 / 50)),
                                   bottomCenterPoint.y() +
                                       0.5 * cosf(deg2Rad(startAngle + i * prop * 360.0 / 50)),
                                   prop);
                    osg::Vec3f loc1 =
                        osg::Vec3f(bottomCenterPoint.x() +
                                       0.5 * sinf(deg2Rad(startAngle + i * prop * 360.0 / 50)),
                    ...
                    osg::Vec3f loc2 = osg::Vec3f(
                        bottomCenterPoint.x() +
                            0.5 * sinf(deg2Rad(startAngle + (i + 1) * prop * 360.0 / 50)),
                        bottomCenterPoint.y() +
                            0.5 * cosf(deg2Rad(startAngle + (i + 1) * prop * 360.0 / 50)),
                        0.0);
                    osg::Vec3f loc3 = osg::Vec3f(
                        bottomCenterPoint.x() +
                            0.5 * sinf(deg2Rad(startAngle + (i + 1) * prop * 360.0 / 50)),
                        bottomCenterPoint.y() +
                            0.5 * cosf(deg2Rad(startAngle + (i + 1) * prop * 360.0 / 50)),
                    ...
                    normals->push_back(vec3ToSphere(loc1) - vec3ToSphere(osg::Vec3(0.5, 0.5, 0.0)));
                    // normals->push_back(vec3ToSphere(loc1) - vec3ToSphere(osg::Vec3(0.5, 0.5,
                    // 0.0)));
                    normals->back().normalize();
                    pGeom->setNormalArray(normals, osg::Array::BIND_OVERALL);
                    pcGeode->addDrawable(pGeom.get());
                ...
                dText->setPosition(vec3ToSphere(osg::Vec3f(
                    bottomCenterPoint.x() + 0.55 * sinf(deg2Rad(startAngle + prop * 360.0 / 2)),
                    bottomCenterPoint.y() + 0.55 * cosf(deg2Rad(startAngle + prop * 360.0 / 2)),
                    prop + 0.1)));
                ...
                pText->setPosition(vec3ToSphere(
                    osg::Vec3f(bottomCenterPoint.x() +
                                   0.65 * sinf(deg2Rad(startAngle + prop * 360.0 / 2 + 5.0)),
                               bottomCenterPoint.y() +
                                   0.65 * cosf(deg2Rad(startAngle + prop * 360.0 / 2 + 5.0)),
                               prop + 0.1)));
  ...
  public:
     ...
     * -- volDat: 体数据，需按Z-Y-X的顺序存放体素
     */
    void AddPieData(const std::string &name,
                    std::shared_ptr<std::vector<std::pair<const wchar_t *, float>>> volPieDat) {
        auto itr = vols.find(name);
        if (itr != vols.end()) {
            param.grp->removeChild(itr->second.geode);
            vols.erase(itr);
        }
        vols.emplace(std::pair<std::string, PerVolParam>(name, PerVolParam(volPieDat, &param)));
        param.grp->addChild(vols.at(name).geode);
    }
    /*
     * 函数: GetVolumes
