  ...
  private:
    ...
    class PerVolParam {
      ...
      public:
        ...
        // unused
        osg::Geode *MakeCylinderChart(const std::array<int32_t, 3> &cooDimMax,
                                      const std::array<int32_t, 3> &cooDimMin,
        ...
        osg::MatrixTransform *CreateCylinder(osg::Vec3f localpoint) {
            auto vec3ToSphere = [&](const osg::Vec3 &v3) -> osg::Vec3 {
                float dlt = maxLongtitute - minLongtitute;
                float x = volStartFromLonZero == 0 ? v3.x()
                          : v3.x() < .5f           ? v3.x() + .5f
            ...
            // osg::Vec3f point = vec3ToSphere(osg::Vec3f(localpoint.x(), localpoint.y(), 0.0));

            osg::Vec3f bottom = vec3ToSphere(osg::Vec3f(localpoint.x(), localpoint.y(), 0.0));
            osg::Vec3f top = vec3ToSphere(localpoint);
            osg::Vec3f center = (top + bottom) / 2.0;

            osg::Vec3f origin = osg::Vec3f(0.0, 0.0, 0.0);

            osg::ref_ptr<osg::Cylinder> cy = new osg::Cylinder;
            cy->setCenter(center);
            cy->setRadius(osg::WGS_84_RADIUS_EQUATOR / 100);
            cy->setHeight(distance(bottom, top));

            // cy->setHeight(osg::WGS_84_RADIUS_EQUATOR / 10);

            // 根据法向量对圆柱体做适当的旋转
            osg::Vec3f targetNormal = bottom - origin;
            osg::Vec3f currentNormal = osg::Vec3f(0.0, 0.0, 0.1);
            // osg::Quat rotationQuat;
            // rotationQuat.makeRotate(currentNormal, targetNormal);

            osg::Matrix mRotate, mTrans, mTrans_v;
            mRotate.makeRotate(currentNormal, targetNormal);
            mTrans.makeTranslate(center);
            mTrans_v.makeTranslate(-center);

            osg::ref_ptr<osg::MatrixTransform> rotationTransform = new osg::MatrixTransform;
            // rotationTransform->setMatrix(osg::Matrix::rotate(rotationQuat));
            rotationTransform->setMatrix(mTrans_v * mRotate * mTrans);
            // rotationTransform->addChild(cy);

            // osg::ref_ptr<osg::ShapeDrawable> pShapeDrawable = new
            // osg::ShapeDrawable(pSphereShape.get(), hints);
            osg::ref_ptr<osg::ShapeDrawable> pShapeDrawable =
                new osg::ShapeDrawable(cy.get(), hints);
            pShapeDrawable->setColor(osg::Vec4f(1.0, 1.0, 1.0, 1.0));
            lcGeode->addDrawable(pShapeDrawable);

            rotationTransform->addChild(lcGeode);
        ...
        //		sphereLoc0 = vec3ToSphere(locaPoint[i - 1]);
        //		sphereLoc1 = vec3ToSphere(locaPoint[i]);
        //		osg::ref_ptr<osg::Sphere> pSphereShape = new osg::Sphere(sphereLoc1,
        //osg::WGS_84_RADIUS_EQUATOR / 100); 		osg::ref_ptr<osg::ShapeDrawable> pShapeDrawable = new
        //osg::ShapeDrawable(pSphereShape.get(), hints); 		pShapeDrawable->setColor(lcColor);
        //		lcGeode->addDrawable(pShapeDrawable);

        //		if (i == 1) {
        //			osg::ref_ptr<osg::Sphere> pSphereShape0 = new osg::Sphere(sphereLoc0,
        //osg::WGS_84_RADIUS_EQUATOR / 100); 			osg::ref_ptr<osg::ShapeDrawable> pShapeDrawable0 = new
        //osg::ShapeDrawable(pSphereShape0.get(), hints); 			pShapeDrawable0->setColor(lcColor);
        //			lcGeode->addDrawable(pShapeDrawable0);
        //			vertices->push_back(sphereLoc0);
        //			colors->push_back(lcColor);
        //		}
        ...
        //	lGeom->setVertexArray(vertices.get());
        //	lGeom->setColorArray(colors.get(), osg::Array::BIND_PER_VERTEX);
        //	lGeom->addPrimitiveSet(new osg::DrawArrays(osg::PrimitiveSet::LINE_STRIP, 0,
        //vertices->size())); 	lcGeode->addDrawable(lGeom);
